			...
			<div id="rr" class="flex relative mx-auto justify-center">
				<article class="w-fit overflow-hidden pb-16 md:px-10 expressive-code-overrides lg:max-w-none ">
					...
					<Show when={titles().title}>
						{(t) => (
							<h1 class="prose-headings text-[2.85rem] text-blue-950 tracking-wide dark:text-white mb-[24px]">
								{t()}
							</h1>
						)}
					</Show>
					<span class="block -mt-[23px] pl-[2px] text-sm opacity-90 font-xs dark:[&>a]:text-slate-600 tracking-wider hover:opacity-1 transition-all [&>a>svg]:w-[12px] xl:hidden">
						<EditPageLink />
					</span>
